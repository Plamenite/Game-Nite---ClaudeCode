...
- Voice: **Agora** (~$0.99/1k user-minutes after 10k free). OFF by default,
  auto-leave when idle, small free daily allowance, VIP unlimited, extra
  minutes for coins, server meters. Wrap the SDK.
...
- Friends (DECIDED): own list (add by code/username) + Facebook importer.
  Party (DECIDED): leader makes a code, friends join, all Ready, leader launches.
...
- `apps/mobile/expo-env.d.ts` is committed so `tsc` works on a fresh clone.
  Root: `npm run mobile|server|test|typecheck` (CI runs the last two on every
  push). Windows setup: `scripts/setup-windows.ps1` (`GAMENITE_SLIM=1`).
...
- Economy LIVE: `game-rules/src/economy.ts` (settleTable), SQL
  ...
  phone Home card. Ludo Star roadmap (streaks, spin, gifts) in ECONOMY.md.
